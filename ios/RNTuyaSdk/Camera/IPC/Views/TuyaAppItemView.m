...
#import "TuyaAppItemView.h"
#import "TuyaAppViewUtil.h"
#import "TuyaAppViews.h"
...
@implementation TuyaAppItemView

+ (TuyaAppItemView *)itemViewWithFrame:(CGRect)frame {
    return [[TuyaAppItemView alloc] initWithFrame:frame];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self addGestureRecognizer:[TuyaAppViewUtil singleFingerClickRecognizer:self sel:@selector(itemViewTap)]];
    }
    return self;
}
...
- (UIView *)customLine:(CGPoint)origin width:(float)width color:(UIColor *)color {
    return [TuyaAppViewUtil viewWithFrame:CGRectMake(origin.x, origin.y, width, 0.5) color:color];
}
...
- (UILabel *)leftLabel {
    if (!_leftLabel) {
        UIView *labelView = [TuyaAppViewUtil viewWithFrame:CGRectMake(0, 0.5, 200, self.height-1) color:nil];
        
        _leftLabel = [TuyaAppViewUtil labelWithFrame:CGRectMake(15, 0, 200, self.height) fontSize:18 color:LIST_MAIN_TEXT_COLOR];
        _leftLabel.adjustsFontSizeToFitWidth = YES;
        
        [labelView addSubview:_leftLabel];
        [labelView addGestureRecognizer:[TuyaAppViewUtil singleFingerClickRecognizer:self sel:@selector(leftLabelTap)]];
...
- (UILabel *)rightLabel {
    if (!_rightLabel) {
        UIView *labelView = [TuyaAppViewUtil viewWithFrame:CGRectMake(self.width - 150, 0.5, 150, self.height-1) color:nil];
        
        _rightLabel = [TuyaAppViewUtil labelWithFrame:CGRectMake(0,0,115, self.height) fontSize:18 color:LIST_MAIN_TEXT_COLOR];
        _rightLabel.textAlignment = NSTextAlignmentRight;
        _rightLabel.adjustsFontSizeToFitWidth = YES;
        
        [labelView addSubview:_rightLabel];
        [labelView addGestureRecognizer:[TuyaAppViewUtil singleFingerClickRecognizer:self sel:@selector(rightLabelTap)]];
        [self addSubview:labelView];
    }
    return _rightLabel;
}
...
- (UILabel *)centerLabel {
    if (!_centerLabel) {
        UIView *labelView = [TuyaAppViewUtil viewWithFrame:CGRectMake((self.width - 150)/2, 0.5, 150, self.height-1) color:nil];
        
        _centerLabel = [TuyaAppViewUtil labelWithFrame:CGRectMake(0,0,150,self.height) fontSize:18 color:LIST_MAIN_TEXT_COLOR];
        _centerLabel.textAlignment = NSTextAlignmentCenter;
        _centerLabel.adjustsFontSizeToFitWidth = YES;
        
        [labelView addSubview:_centerLabel];
        [labelView addGestureRecognizer:[TuyaAppViewUtil singleFingerClickRecognizer:self sel:@selector(centerLabelTap)]];
        [self addSubview:labelView];
    }
    return _centerLabel;
}
...
- (void)setLeftImage:(UIImage *)leftImage {
    ...
    [_leftImageView addGestureRecognizer:[TuyaAppViewUtil singleFingerClickRecognizer:self sel:@selector(leftImageViewTap)]];
    [self addSubview:_leftImageView];
}

- (void)setCenterImage:(UIImage *)centerImage {
    _centerImageView = [self imageViewForItemView:TPItemViewImagePositionCenter image:centerImage];
    [_centerImageView addGestureRecognizer:[TuyaAppViewUtil singleFingerClickRecognizer:self sel:@selector(centerImageViewTap)]];
...
- (void)setRightImage:(UIImage *)rightImage {
    if (!_rightImageView) {
        ...
        [_rightImageView addGestureRecognizer:[TuyaAppViewUtil singleFingerClickRecognizer:self sel:@selector(rightImageViewTap)]];
