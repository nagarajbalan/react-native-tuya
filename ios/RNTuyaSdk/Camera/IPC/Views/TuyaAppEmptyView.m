...
@interface TuyaAppEmptyView()

@property (nonatomic, strong) UILabel *subTitleLabel;

@end

@implementation TuyaAppEmptyView
...
- (void)addEmptyView:(NSString *)title imageName:(NSString *)imageName {
    ...
    UILabel *titleLabel = [TuyaAppViewUtil labelWithFrame:CGRectMake(0, emptyImageView.bottom + 20, self.width, 0) fontSize:12 color:HEXCOLOR(0x8A8E91)];
...
- (id)initWithFrame:(CGRect)frame title:(NSString *)title subTitle:(NSString *)subTitle {
    if (self = [super initWithFrame:frame]) {
        ...
        UILabel *titleLabel = [TuyaAppViewUtil simpleLabel:CGRectMake(0, 0, self.width, 33) bf:24 tc:HEXCOLOR(0x303030) t:title];
        titleLabel.numberOfLines = 1;
        titleLabel.textAlignment = NSTextAlignmentCenter;
        [self addSubview:titleLabel];
        
        
        UILabel *subTitleLabel = [TuyaAppViewUtil simpleLabel:CGRectMake(0, titleLabel.bottom + 16, self.width, 0) f:12 tc:HEXCOLOR(0x8A8E91) t:subTitle];
