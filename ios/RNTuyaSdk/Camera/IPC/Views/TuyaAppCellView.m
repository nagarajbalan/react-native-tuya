...
#import "TuyaAppCellView.h"
#import "TuyaAppTheme.h"

@implementation TuyaAppCellView

+ (TuyaAppCellView *)seperateCellView:(CGRect)frame  backgroundColor:(UIColor *)backgroundColor {
    TuyaAppCellView *seperateCellView = [[TuyaAppCellView alloc] initWithFrame:frame];
...
- (void)setDelegate:(NSObject<TuyaAppCellViewDelegate> *)delegate {
    if (!_delegate) {
        [self addGestureRecognizer:[TuyaAppViewUtil singleFingerClickRecognizer:self sel:@selector(viewCellTap)]];        
    }
    _delegate = delegate;
}
...
- (void)setRightItem:(TuyaAppCellViewItem *)rightItem {
...
- (void)showRightArrow {
    if (!_rightItem) {
        _rightItem = [[TuyaAppCellViewItem alloc] init];
    }
